                ...
                <div class="row">
                	...
                	<div class="col-sm-12">
                		<div class="form-group">
                			<label>Title</label>
                			<input required="required" type="text" name="title" value="<?php echo isset($question->title)?$question->title:'';?>" placeholder="" class="form-control">
                		</div>
                	</div>
                	<div class="col-sm-12">
                		<div class="form-group">
                			...
                			<!-- <input required="required" type="text" name="title" value="<?php echo isset($question->title)?$question->title:'';?>" placeholder="" class="form-control"> -->
                			<textarea name="form_description" class="form-control"><?php echo isset($question->form_description)?$question->form_description:'';?></textarea>
                		</div>
                	</div>
                	...
                	<div class="col-sm-10">
                		<div class="form-group">
                			...
                			<?php 
                			$form_data = isset($question->form_data)?$question->form_data:'';
								$form_data_ar=unserialize($form_data);
								$form_data_questions = isset($form_data_ar['questions'])?$form_data_ar['questions']:[];
								$form_data_remarks = isset($form_data_ar['remarks'])?$form_data_ar['remarks']:[];
								/*echo '<pre>';
								print_r($form_data_questions);
								print_r($form_data_remarks);
								echo '</pre>';*/
...
$question_idx = explode(',',$questions_string);
                			...
                			?>
                			<select id="question_list" class="form-control" name="idx[]">
                				<?php if(isset($questions) && count($questions)>0){
                					foreach($questions as $ques){
                							if(!in_array($ques->id,$form_data_questions)){
                						?>
                				<option <?php if(isset($ques->id) && in_array($ques->id,$question_idx)){echo 'selected';}?> value="<?php echo isset($ques->id)?$ques->id:0;?>"><?php echo isset($ques->title)?$ques->title:'';?></option>
                				<?php }}}?>
                	...
                	<div class="col-sm-2">
						...
						<button type="button" class="btn btn-primary btn_add_question" style="margin-top: 26px;">Add Question</button>
                	</div>
                	<div class="clearfix"></div>
							...
							<tbody id="question_box">
								...
								$LO=0;
								 if(isset($questions) && count($questions)>0){
                					foreach($questions as $ques){
                						if(in_array($ques->id,$form_data_questions)){
                							$K = array_search($ques->id, $form_data_questions);
                							$remark_checked = '';
                							if(isset($form_data_remarks[$K])){
                								$remark_checked='checked';
                							}
                							?>
<tr><td class="name"><?php echo $ques->title;?><input class="h_id" type="hidden" name="box_id[<?php echo $LO;?>]" value="<?php echo $ques->id;?>"></td><td><input <?php echo $remark_checked;?> data-switch-no-init type="checkbox" name="remarks[<?php echo $LO;?>]" value="1"></td><td><button type="button" class="btn btn-sm btn-danger delete_question_table">Delete</button></td></tr>
                						<?php $LO++;}}}?>
                	...
                	<div class="col-sm-12 hidden">
                		<div class="form-group">
                			...
                			<?php 
$form_id = isset($question->id)?$question->id:'-';
                			...
                			<select class="form-control select2" multiple="multiple" name="employee_idx[]">
                				<?php if(isset($employees) && count($employees)>0){
                					foreach($employees as $emp){
                						...
                						?>
                				<option <?php if(isset($emp->id) && $emp->form_id==$form_id){echo 'selected';}?> value="<?php echo isset($emp->id)?$emp->id:0;?>"><?php echo $userData;?></option>
                				<?php }}?>
			 ...
			 <div class="form-group">
            	<label><input <?php if(isset($question->normal_distribution) && $question->normal_distribution==1){echo 'checked';}?> value="1" data-switch-no-init type="checkbox" name="normal_distribution"> Apply Normal Distribution</label>
            </div>
            <div class="clearfix"></div>
...
<script>
	var glob_counter=<?php echo $LO;?>;
	$(document).on('click','.delete_question_table',function(){
		var selected_text = $(this).closest('tr').find('.name').text();
		var selected_id = $(this).closest('tr').find('.h_id').val();
		$('#question_list').select2('destroy');
		$('#question_list').append('<option value="'+selected_id+'">'+selected_text+'</option>');
	...
	$(document).on('click','.btn_add_question',function(){
		$('#question_list').select2('destroy');
		var selected_id = $('#question_list').val();
		
		var selected_text = $('#question_list option:selected').text();
		$('#question_list option:selected').remove();
		
		$('#question_list').select2();
		if(selected_id>0){
		var html='<tr><td class="name">'+selected_text+'<input class="h_id" type="hidden" name="box_id['+glob_counter+']" value="'+selected_id+'"></td><td><input type="checkbox" name="remarks['+glob_counter+']" value="1"></td><td><button type="button" class="btn btn-sm btn-danger delete_question_table">Delete</button></td></tr>';
	...
	$(document).on('change blur',".quantity,.dollar_price,.sale_price,.per_unit",function(){
        ...
        $('#submit-order').on('click', function () {
            ...
            $("form#form-order").validate({
                ...
                submitHandler: function (form) {
                    var total_length = $('#question_box tr').length;
                	if(total_length>0){
                		form.submit();	
                	}else{
